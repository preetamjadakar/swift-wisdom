...
do {
    let decoded = try decoder4.decode([User41].self, from: data4)
    print(decoded[0])
} catch {
    print("Failed to decode JSON41", error)
}
do {
    let decoded = try decoder4.decode([User42].self, from: data4)
    print(decoded[0].name)
} catch {
    print("Failed to decode JSON42", error)
}
